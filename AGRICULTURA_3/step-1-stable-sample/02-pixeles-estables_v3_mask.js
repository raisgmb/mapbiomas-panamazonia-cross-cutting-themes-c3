...
var exclusion_30203_1 = /* color: #d63000 */ee.FeatureCollection(
    ...
    natural85 = /* color: #d63000 */ee.FeatureCollection(
        [ee.Feature(
            ee.Geometry.Polygon(
                [[[-68.86792389370642, -11.172951467912991],
                  [-68.86363238419501, -11.17648799033138],
                  [-68.8565943086146, -11.17598277548764],
                  [-68.85350442172705, -11.165036237676706],
                  [-68.8514444970929, -11.153584033170981],
                  [-68.85693762931089, -11.150215651777193],
                  [-68.86723725353573, -11.153584031199989],
                  [-68.86929717721343, -11.156615542032418],
                  [-68.8679238937144, -11.170593762336916]]]),
            {
              "original": "1,",
              "new": "0,",
              "system:index": "0"
...
var param = {
  ...
  exclusion : {
    ...
    classes: [],
    NoExcluirClass:[],// no excluye las clases de los pixeles estables a pesar de la máscara
    polygons: [ natural85, natural86, natural87, geometry ],
    //shape: 'natural_85_shp',
    //shape: 'TIERRA_AGROPECUARIA_2018_40201',
    shape: '',
  },
...
(function init(param) {
  ...
  fullRegion = fullRegion.mask(stablePixels.gt(0))
  var NewMask = excludeAreas(fullRegion, stablePixels, shapePath, shapeName);
  
  stablePixels = stablePixels.mask(NewMask.gt(0))
 ...
 */
function excludeAreas(mask,image, shapePath, shapeName) {
  ...
  if(shapes){
    ...
    Map.addLayer(exclusionRegions,{},'Shape exlusion',false);
  }
  ...
  if(exclusionRegions !== null) {
    ...
    exclusionRegions = exclusionRegions
      ...
      .eq(1);
    var ImgNoExclusion = image.multiply(0)
    
    if (param.exclusion.NoExcluirClass.length>0){
      param.exclusion.NoExcluirClass.forEach(function(ele){
        //print('Clase',ele)
        ImgNoExclusion = ImgNoExclusion.where(image.eq(ele),ele)
        
      })
    }
    mask = mask.where(exclusionRegions.eq(1), 0)
    mask = mask.where(ImgNoExclusion.gt(0),1)
    mask = mask.mask(mask.gt(0))
    return mask
  } 
  else return mask;
}
  ...
  if(polygons.length > 0) {
    polygons.forEach(function( polygon ) {
      ...
      var excluded = polygon.map(function( layer ){
       ...
       // print(param.exclusion.NoExcluirClass.indexOf(from))
        
        if (param.exclusion.NoExcluirClass.indexOf(from)===-1){
          area.remap(from, to)
        }
        //return area;
...
function getStablePixels2(image, classes) {
  
  var bandNames = [],//image.bandNames(),
      images = [];
  for (var i =1985;i<=2018;i++){
    bandNames[i-1985] = 'classification_'+i
  }
  var Nro = bandNames.length;
  var Ini =0;
  var ImgTemp01;
  var Resultado = image.select(0).multiply(0)
  var ImgAnt = image.select(0);
  
  bandNames.forEach(function(Ele){
    ImgTemp01 = image.select(Ele)
    Resultado = Resultado.add(ImgTemp01.eq(ImgAnt))
    ImgAnt = ImgTemp01;
  })
  Resultado = Resultado.mask(Resultado.gte(Nro))
  Resultado = Resultado.where(ImgAnt.gt(0),ImgAnt)

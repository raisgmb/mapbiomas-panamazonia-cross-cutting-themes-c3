...
if(param.piramideActive) {
  ...
  Map.addLayer(img.select(bandNames), vis3, 'class-ORIGINAL'+param.year);
  Map.addLayer(fil, vis3, 'class-SPATIAL FILTER'+param.year);
} else {
  Map.addLayer(mosaic.filterMetadata('year', 'equals', param.year)
                     .mosaic()
                     .updateMask(regionRaster), {
  ...
  Map.addLayer(img.select(bandNames)
                       .reproject('EPSG:4326', null, 30), vis3, 'class-ORIGINAL'+param.year);
  Map.addLayer(fil.reproject('EPSG:4326', null, 30), vis3, 'class-SPATIAL FILTER'+param.year);
}

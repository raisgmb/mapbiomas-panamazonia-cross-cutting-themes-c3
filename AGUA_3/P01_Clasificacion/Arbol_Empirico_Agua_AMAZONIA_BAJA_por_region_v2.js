...
var shademask2_v1 = ee.Image("projects/mapbiomas-raisg/MOSAICOS/shademask2_v1"),
    ...
    exclusion = /* color: #ff0000 */ee.FeatureCollection(
        ...
        ee.Feature(
            ee.Geometry.Polygon(
                [[[-78.28673045069152, -5.168620031960578],
                  [-78.30235163599426, -5.146223433487669],
                  [-78.30664317041808, -5.14229112480775],
                  [-78.32037608057433, -5.1467363415239715],
                  [-78.33857218653137, -5.154087977912401],
                  [-78.3402888003009, -5.163491109729389],
                  [-78.34681193262512, -5.184519426774984]]]),
            {
              "value": 1,
              "system:index": "3"
            }),
...
var param = {
    code_region: 70203,  //Region de Clasificacion
    carta : '',
    pais  :'PERU',
    version:'1',
    inclusion: inclusion,
    ndwi_mf_inclu: 30,
    exclusion: exclusion,
    inclusionRios: 'projects/mapbiomas-raisg/DATOS_AUXILIARES/RASTERS/per_rios_amazonia', // CAPA RIOS valores >= 1
    Buffer_rios :90, //Buffer en metros
    };
...
//inclusion rios buffer
  var inclu_rios = ee.Image(param.inclusionRios).gte(1)
  var River_buffer = ee.Image(1)
    .cumulativeCost({
      source: inclu_rios, 
      maxDistance: param.Buffer_rios,
    }).lt(param.Buffer_rios);
  
  River_buffer = ee.Image(0).where(River_buffer.eq(1), 1).updateMask(region_raster)
  Map.addLayer(River_buffer.selfMask(),{palette:['ff0000']},'River_buffer')
...
YearsParam.forEach(function(parametro){
  ...
  classification_treeD = classification_treeD.where(exclu.eq(1), 27)
                                             ...
                                             .where(River_buffer.eq(1).and(mosaic.select('ndwi_mcfeeters_median').gte(param.ndwi_mf_inclu)), 33)

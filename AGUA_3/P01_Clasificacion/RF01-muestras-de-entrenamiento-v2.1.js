...
var param = {
  ...
  version_out: '1',
  displayPoints: 500 // Puntos a mostrar
};
...
var SampleWater = function(param) {
...
this.inputs = {
  ...
  _samples: 'projects/mapbiomas-raisg/MUESTRAS/COLECCION3/TRANSVERSALES/AGUA/',
  references: {
      mapbiomas: 'projects/mapbiomas-raisg/SUBPRODUCTOS/MOORE/classification/mapbiomas-raisg-collection20-integration-v8',
      peru: '',
      colombia: '',
      ecuador: '',
      bolivia: '',
...
this.init = function(param) {
  ...
  var nodata = stablePixels.neq(33);  // revisar
  var water  = stablePixels.eq(33);  // revisar
  ...
  years.forEach(function(year){
      ...
      // var fileName = 'water' + '-' + regionId + '-' + country + '-' + year + '-' + version_out;
  ...
  // Export samples to asset
  var fileName = 'water' + '-' + regionId + '-' + country  + '-' + version_out;
  var assetId = samplesAsset + fileName;
  ...
  // Layers
    ...
    var subsetPoints = ee.FeatureCollection(
      pts.filterMetadata('reference', 'not_equals', 33)
        .toList(displayPoints)
    ).merge(
      ee.FeatureCollection(
        pts.filterMetadata('reference', 'equals', 33)
          .toList(displayPoints)
      )
    ); 
    
    var trainingPointsColor = subsetPoints.map(
        function (feature) {
...
var Samples = new SampleWater(param);
//Filtro espacial Agua 
var param = {  
    code_region: 70201,  //Region de Clasificacion 
    pais: 'PERU', 
...
var prefixo_out = param.pais+ '-' + param.code_region + '-' + param.metodo + '-' + param.version_output
...
// restrigir Rios
var dist_buffer = param.buffer_rios
...
OSM_buffer = ee.Image(0).where(OSM_buffer.eq(1), 1).updateMask(regionRaster)


var Class_origin_rios = class4FT.select(bandNames).updateMask(OSM_buffer.eq(1)).selfMask()

class_outTotal = class_outTotal.select(bandNames).blend(Class_origin_rios)
...
Map.addLayer(OSM_buffer.selfMask(),{palette:['25ffff']},'OSM_buffer',false);

...
var param = { 
    ...
    KernelTemporal:[  //Filtros Solo Años Intermedios
      'window3years',
      'window4years',
      // 'window5years',
      'window3years',
...
var ordem_exec_first =  [33,27];              //Filtro de primer año
var ordem_exec_last =   [33,27];                               //Filtro de ultimo año
var ordem_exec_middle = [33,27,33,27];  //Filtro de años intermedios

//--------------------------------------------
var dirinputOrig = "projects/mapbiomas-raisg/TRANSVERSALES/AGUA_3/clasificacion";
...
var prefixo_out = param.pais+ '-' + param.code_region + '-' + param.metodo + '-';
...
if(param.version_input == '1'){
    ...
    Clasificacion_TD = ee.ImageCollection(dirinputOrig)
    ...
    Clasificacion_TD = ee.ImageCollection(dirinput)
...
// Filtros Años Intermedios
if(param.KernelTemporal.length !== 0){
  for (var ii=0; ii<param.KernelTemporal.length; ii++) {
  
      print(param.KernelTemporal[ii])
  if(param.KernelTemporal[ii]=='window3years'){
  ...
  if(param.KernelTemporal[ii]=='window4years'){
      for (var i_class=0;i_class<ordem_exec_middle.length; i_class++){  
           var id_class = ordem_exec_middle[i_class]; 
            filtered = window4years(filtered, id_class)
          }
  }
  if(param.KernelTemporal[ii]=='window5years'){
      for (var i_class=0;i_class<ordem_exec_middle.length; i_class++){  
       var id_class = ordem_exec_middle[i_class]; 
